...
void loop() {
  ...
  switch (mode) {
    case 0:
      ...
      // desiredRhoVel = 0;
      // Only set angle from PI once
      // if (lastMode == 2) {
      //   //desiredPhi = markerAngle; //This was commented
      // }
      // // firstFlagPhi = false;
      // // If phi within 3 degrees switch modes
      if ((phi <= (desiredPhi + (2*PI/180))) && (phi >= (desiredPhi+(-2*PI/180)))) {
        if (lastMode == 1) {
          lastMode = 0;
          mode = 4;
    ...
    break;
      case 1:
        ...
        if (first_flag == true) {
          desiredRho = 2-0.4; // driving approximately seven feet
        }
        first_flag = false;
      ...
      case 4: // Spin in circle
        ...
        desiredRhoVel = 5;
        desiredPhiVel = desiredRhoVel/(0.35*1.5);// rho/(rad of circle)
        // Only spin for 9.35 seconds
        if (currentTime-startCircleTime >= 9.35) {
          desiredRhoVel = 0;
          desiredPhiVel = 0;
          // Turn off when done
          mode = 3;
